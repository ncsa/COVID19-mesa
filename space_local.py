...
def accept_tuple_argument(wrapped_function):
    ...
    #Subclass representing an unit within space
...
class Grid:
    ...
    def __init__(self, width, height, torus):
        ...
        for x in range(self.width):
            ...
            self.movement_options.append(movement_col)
    ...
    """Given a grid equal in dimensions as the current grid containing a probability vector, append it to the cell class."""
    def apply_vspace(self, vspace,unit_space):
        for x in range(self.width):
            for y in range(self.height):
                movements = []
                chances = []
                directions = vspace[x][y]
                for y_, row in enumerate(directions):  # Looking through the directions and retrieving the nonzero directions TODO move this to space
                    for x_, chance in enumerate(row):
                        if chance > 0:
                            movements.append((x_ - 1, -1 * y_ + 1))
                            chances.append(chance)
                options = [movements, chances, unit_space[x][y]]
                self.movement_options[x][y] = options
    ...
    def iter_cell_list_contents(self, cell_list):
        ...
        return (self[x][y].agents for x, y in cell_list if not self.is_cell_empty((x, y)))
    ...
    def _remove_agent(self, pos, agent):
        ...
        x, y = pos
        if self.grid[x][y]: #Checks if the agent list is not empty
...
class MultiGrid(Grid):
    ...
    """Removed the altered removal and replacement as using a list generalizes it for Single and Multigrid"""
    """If the runtime is significantly slower for single grids, then we can create a special case for it."""
